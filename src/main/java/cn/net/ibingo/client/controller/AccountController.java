...
public class AccountController extends BaseController {
    ...
    @ResponseBody
    public JSONArray userRegistered(User user, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (user != null && !StringUtils.isEmpty(user.getUserName()) && !StringUtils.isEmpty(user.getPassWord()) &&
            ...
            if (CheckUtil.stringLength(user.getUserName(), 255) && CheckUtil.stringLength(user.getPassWord(), 100) && CheckUtil.stringLength(user.getImsi(), 100)
                ...
                //设置默认头像
                user.setImageUrl(ConstantConfig.IMAGESPATH+ File.separator+"default.jpg");
                //注册
                int count = userService.insertUser(user);
                if (count > 0) {
                    //注册成功后直接登录用户
                    JSONArray jsonArray = commonUserLogin(user, request, response, 0);
    ...
    @ResponseBody
    public JSONArray commonUserLogin(User user, HttpServletRequest request, HttpServletResponse response, int loginType) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        //判断上传数据是否为空
        if (user != null && !StringUtils.isEmpty(user.getUserName()) && !StringUtils.isEmpty(user.getPassWord())) {
            //判断用户账号是手机号码还是邮箱地址 并进行校验
            ...
            if (oldUser != null) {
                ...
                //保存登录日志
                saveLoginLog(oldUser.getUserName(), loginType);

                resultMap.put("userId", oldUser.getUserId());
                resultMap.put("userName", oldUser.getUserName());
                resultMap.put("nickName", oldUser.getNickName());
                resultMap.put("sex", oldUser.getSex());
                resultMap.put("birthDate", oldUser.getBirthDate());
                resultMap.put("country", oldUser.getCountry());
                resultMap.put("image", "");
                resultMap.put("status", ConstantConfig.HTTP_CODE_1000);
                return JSONArray.fromObject(resultMap);
            } else {
    ...
    @ResponseBody
    public JSONArray thirdUserLogin(User user, HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        //判断参数是否为空
        if (user != null && !StringUtils.isEmpty(user.getUserId())) {
            ...
            if (oldUser == null) {
                ...
                if (count > 0) {
                    ...
                    resultMap.put("birthDate", user.getBirthDate());
                    resultMap.put("country", user.getCountry());
                ...
                resultMap.put("birthDate", oldUser.getBirthDate());
                resultMap.put("country", oldUser.getCountry());
                resultMap.put("imageUrl", oldUser.getImageUrl());
                resultMap.put("status", ConstantConfig.HTTP_CODE_1000);
                return JSONArray.fromObject(resultMap);
            }
        } else {
    ...
    @ResponseBody
    public JSONArray sendCode(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
            ...
            if (username.indexOf("@") > -1) {
                ...
                if (flg) {
                    try {
                        ...
                        EmailUtil.sendMessage(username, ConstantConfig.EMAIL_SUBJECT, ramNum + ConstantConfig.EMAIL_SUBJECT);
                    } catch (Exception e) {
                        e.printStackTrace();
                        resultMap.put("status", ConstantConfig.HTTP_CODE_1003);
                        return JSONArray.fromObject(resultMap);
    ...
    @ResponseBody
    public JSONArray updatePassword(User user, HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        String verificCode = request.getParameter("verificCode");
        if (user != null && !StringUtils.isEmpty(user.getUserName()) && StringUtils.isEmpty(user.getPassWord()) && !StringUtils.isEmpty(verificCode)) {
            VerificCodeRedisModel vr = new VerificCodeRedisModel();
            vr.setUsername(user.getUserName());
            vr.setRamNum(verificCode);
            //从redis获取该用户对应的验证码
            String redisVerificCode = verificCodeRedisFactory.get(vr);
    ...
    @ResponseBody
    public JSONArray updateUserInfo(User user, HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        //判断上传数据是否为空
    ...
    @ResponseBody
    public JSONArray updateUserInfo(HttpServletRequest request, HttpServletResponse response, @RequestParam(value = "file", required = false) MultipartFile file) {//,@RequestParam(value = "file",required = false)MultipartFile file
        Map<String, Object> resultMap = new HashMap<String, Object>();
        //判断上传数据是否为空
    ...
    @ResponseBody
    public JSONArray downloadImage(HttpServletRequest request, HttpServletResponse response) {
        Map<String, Object> resultMap = new HashMap<String, Object>();
        String fileName = request.getParameter("userName");
        if(!StringUtils.isEmpty(fileName)){
            FileUtil.imageDownload(request, response);
        }else{
            resultMap.put("status", ConstantConfig.HTTP_CODE_1002);
            return JSONArray.fromObject(resultMap);
        }
     ...
     * @return
     */
    public Map<String, Object> createSession(User oldUser, HttpServletRequest request, HttpServletResponse response, Map<String, Object> resultMap) {
        HttpSession session = request.getSession();
        // 1、创建cookie
        try {
            oldUser.setUserName(URLEncoder.encode(oldUser.getUserName(), "utf-8"));
     ...
     * @param loginType
     */
    public void saveLoginLog(String username, int loginType) {
        //保存登录日志
